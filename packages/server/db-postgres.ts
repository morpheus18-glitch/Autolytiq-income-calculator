...
export const affiliateDb = {
  ...
  // Credit Karma specific queries
  getCreditKarmaStats: async (startDate: string, endDate: string) => {
    return queryOne<{ total_clicks: number; unique_sessions: number }>(
      `SELECT COUNT(*) as total_clicks, COUNT(DISTINCT session_id) as unique_sessions
       FROM affiliate_clicks
       WHERE affiliate_name = 'Credit Karma'
       AND clicked_at >= $1::date AND clicked_at < ($2::date + interval '1 day')`,
      [startDate, endDate]
    );
  },

  getCreditKarmaDailyClicks: async (startDate: string, endDate: string) => {
    return query<{ date: string; clicks: number; unique_sessions: number }>(
      `SELECT DATE(clicked_at) as date, COUNT(*) as clicks, COUNT(DISTINCT session_id) as unique_sessions
       FROM affiliate_clicks
       WHERE affiliate_name = 'Credit Karma'
  ...
  getCreditKarmaByPage: async (startDate: string, endDate: string) => {
    return query<{ page_source: string; clicks: number }>(
      `SELECT page_source, COUNT(*) as clicks
       FROM affiliate_clicks
       WHERE affiliate_name = 'Credit Karma'
  ...
  getCreditKarmaByDevice: async (startDate: string, endDate: string) => {
    return query<{ device_type: string; clicks: number }>(
      `SELECT COALESCE(device_type, 'unknown') as device_type, COUNT(*) as clicks
       FROM affiliate_clicks
       WHERE affiliate_name = 'Credit Karma'
